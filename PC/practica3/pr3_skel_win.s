...
main:
            ...
            lw $s2,mat      #mat -> $s2
            lw $s1,size     #size -> $s1 = 4
            lw $s0,ncol     #ncol -> $s0
            lw $s3,nfil
            move $t0, $zero        #f = 0 -> $t0 

for1:       move $t1, $zero        #c = 0 -> $t1          

for2:       mul  $t4, $t0, $s0              #$t4 = i*ncol
            addu $t4, $t4, $t1              #$t4 = i*ncol+j
            mul  $t4, $t4, $s1              #$t4 = (i*ncol+j)*size
            addu $t4, $t4, $s2              #$t4 direccion matriz [i, j]
            lw   $t3, 0($t4)                #$t4 elemento matriz

            #Imprimir elemento calculado de la matriz
